...
namespace Manager
{
    /// <summary>
    /// 단순 로딩 매니저 (Photon 분리)
    /// - 로컬 씬 로드용 ShowThenLoadLocal
    /// - 임의 액션(예: PhotonNetwork.LoadLevel)을 호출하는 FadeInThenAction
    /// - 씬 로드 완료시 페이드아웃
    /// </summary>
    public class LoadingScreenManager : SingletonDontDestroy<LoadingScreenManager>
    {
        ...
        // 2) 페이드인 후 임의 액션 호출 (PhotonNetwork.LoadLevel 같은 네트워크 호출을 여기서 실행)
        ...
        private IEnumerator FadeInThenActionRoutine(Action onFadeInComplete)
        {
            ...
            // 씬 로드는 호출자(onFadeInComplete)에서 수행 -> OnSceneLoaded에서 페이드아웃 처리
        }

        // 페이드 인 상태로 씬이 로드되면(로컬 또는 네트워크) 자동으로 페이드아웃
